<x-layout heading="Your Profile" bgImage="images/comic-lightning.jpg">
    ...
    <div class="w-full max-w-4xl mx-auto mb-8">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6 ring-1 ring-black/10 shadow-xl">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-white/70 text-xs mt-1">Member since {{ auth()->user()->created_at->format('M Y') }}</p>
                    </div>
                </div>
    ...
    <div class="min-h-[50vh] flex items-center justify-center">
        <div class="w-full max-w-2xl grid gap-8 md:grid-cols-3">
            <div class="md:col-span-2 bg-white/95 dark:bg-gray-900/80 backdrop-blur ring-1 ring-black/5 dark:ring-white/10 shadow-lg rounded-2xl p-6">
            ...
            <div class="md:col-span-1 bg-white/95 dark:bg-gray-900/80 backdrop-blur ring-1 ring-black/5 dark:ring-white/10 shadow-lg rounded-2xl p-6 h-fit">
                ...
                <form method="POST" action="{{ route('profile.destroy') }}" class="space-y-3">
                    ...
                    @method('DELETE')
                    <div>
                        <label for="delete_current_password" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Current
                            Password</label>
                ...
                <ul class="space-y-4">
                    @foreach ($questions as $q)
                        <li class="p-4 bg-white dark:bg-gray-800 rounded-xl ring-1 ring-black/5 dark:ring-white/10">
                            <a href="{{ route('questions.show', $q) }}" class="font-medium text-blue-700 dark:text-blue-400 hover:underline">{{ $q->title }}</a>
                            <div class="mt-2 flex flex-wrap gap-2">
                                @foreach ($q->tags as $t)
                                    <a href="{{ route('tags.show', $t) }}" class="px-2 py-0.5 text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-200 rounded-full">{{ $t->name }}</a>
                                @endforeach
                            </div>
                            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $q->created_at->diffForHumans() }} • {{ $q->answers_count }} answers</div>
                        </li>
                    @endforeach
                </ul>
                <div class="mt-4">{{ $questions->appends(request()->except('questions_page'))->links('pagination::tailwind') }}</div>
            @else
                <p class="text-sm text-gray-600 dark:text-gray-400">No questions yet.</p>
            @endif
        </div>

        <div class="bg-white/95 dark:bg-gray-900/80 backdrop-blur ring-1 ring-black/5 dark:ring-white/10 shadow-lg rounded-2xl p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Your Answers</h3>
            @if(isset($answers) && $answers->count())
                <ul class="space-y-4">
                    @foreach ($answers as $a)
                        <li class="p-4 bg-white dark:bg-gray-800 rounded-xl ring-1 ring-black/5 dark:ring-white/10">
                            <div class="text-sm text-gray-700 dark:text-gray-200">{{ \Illuminate\Support\Str::limit($a->body, 140) }}</div>
                            <a href="{{ route('questions.show', $a->question) }}" class="text-xs text-blue-600 dark:text-blue-400 hover:underline">View question: {{ $a->question->title }}</a>
                            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $a->created_at->diffForHumans() }} @if($a->is_best)<span class="ml-1 px-1.5 py-0.5 rounded bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-200">Best</span>@endif</div>
                        </li>
                    @endforeach
                </ul>
                <div class="mt-4">{{ $answers->appends(request()->except('answers_page'))->links('pagination::tailwind') }}</div>
            @else
    ...
    <div class="w-full max-w-4xl mx-auto mt-6 bg-white/95 dark:bg-gray-900/80 backdrop-blur ring-1 ring-black/5 dark:ring-white/10 shadow-lg rounded-2xl p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Your Comments</h3>
        @if(isset($comments) && $comments->count())
            <ul class="space-y-4">
                @foreach ($comments as $c)
                    <li class="p-4 bg-white dark:bg-gray-800 rounded-xl ring-1 ring-black/5 dark:ring-white/10">
                        <div class="text-sm text-gray-700 dark:text-gray-200">{{ \Illuminate\Support\Str::limit($c->body, 160) }}</div>
                        @if($c->answer && $c->answer->question)
                            <a href="{{ route('questions.show', $c->answer->question) }}" class="text-xs text-blue-600 dark:text-blue-400 hover:underline">On question: {{ $c->answer->question->title }}</a>
                        @endif
                        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $c->created_at->diffForHumans() }}</div>
                    </li>
                @endforeach
            </ul>
            <div class="mt-4">{{ $comments->appends(request()->except('comments_page'))->links('pagination::tailwind') }}</div>
        @else
