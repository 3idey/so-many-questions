<x-layout heading="Question Details">
    <div class="max-w-4xl mx-auto space-y-8">
            ...
            <section class="p-6 bg-white dark:bg-gray-900 rounded-2xl shadow border border-gray-200 dark:border-gray-800">
                ...
                <form method="POST" action="{{ route('answers.store', $question) }}" class="space-y-3">
                    ...
                    <x-input-section label="Answer" for="answer_body" error="body">
                        <textarea id="answer_body" name="body" rows="6" class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Write your answer here...">{{ old('body') }}</textarea>
                    </x-input-section>
        ...
        <section class="space-y-4">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Answers ({{ $question->answers->count() }})</h2>
            ...
            @forelse ($question->answers as $answer)
                <div class="p-5 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                                ...
                                @if ($answer->is_best)
                                    <span class="ml-2 px-2 py-0.5 text-xs rounded bg-green-100 text-green-700">Best</span>
                                @endif
                            </div>
                            <p class="mt-2 text-gray-800 dark:text-gray-200 whitespace-pre-line">{{ $answer->body }}</p>
                        </div>
                        @auth
                    ...
                    <div class="mt-4 border-t border-gray-200 dark:border-gray-700 pt-4">
                        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Replies ({{ $answer->comments->count() }})</h3>
                        <div class="space-y-3">
                            @foreach ($answer->comments as $comment)
                                <div class="text-sm p-3 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                        ...
                        @auth
                            <form method="POST" action="{{ route('comments.store', $answer) }}" class="mt-3 space-y-2">
                                ...
                                <x-input-section label="Add a reply" for="comment_body_{{ $answer->id }}" error="body">
                                    <textarea id="comment_body_{{ $answer->id }}" name="body" rows="3" class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Write a reply..."></textarea>
                                </x-input-section>
